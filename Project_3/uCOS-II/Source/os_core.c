...
static  void  OS_SchedNew (void)
{
	...
	for (int i = 0; i < OS_LOWEST_PRIO; i++)
	{
		...
		{
			OS_TCB *tmp = OSTCBPrioTbl[i];

			if (tmp->OSTCBDly == 0)
			{
				if (tmp->OSTCBdeadline <= OSTime)
				{
					printf("%d      Timeout      %d\n", tmp->OSTCBdeadline - 1, i);
					tmp->OSTCBdeadline += tmp->OSTCBperiod;
				}
				else if (tmp->OSTCBdeadline < mostUrgentDeadline)
				{
					mostUrgentDeadline = tmp->OSTCBdeadline;
					mostUrgentPriority = tmp->OSTCBPrio;
				}
			}
		}
	}
	...
	for (int i = 0; i < OS_LOWEST_PRIO; i++)
	{
		if (OSTCBPrioTbl[i] != NULL)
		{
			...
			if (tmp->OSTCBDly == 0)
			{
				...
				else if (tmp->OSTCBperiod < mostUrgentPeriod)
				{
					mostUrgentPeriod = tmp->OSTCBperiod;
					mostUrgentPriority = tmp->OSTCBPrio;
				}
			}
		}
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
		...
		ptcb->OSTCBcomptime		 = p[COMP_TIME];
		ptcb->OSTCBdeadline		 = p[DEADLINE];
		ptcb->OSTCBrefcomptime   = p[COMP_TIME];
